...
class MyNet(nn.Module):
    def __init__(self, in_channel=1, channel1=32, channel2=64,channel3=128, num_classes=10):
        
        super(MyNet, self).__init__()
        
        self.conv1 = nn.Conv2d(in_channel, channel1, kernel_size=3)
        #self.pool1 = nn.MaxPool2d(2)
        self.conv2 = nn.Conv2d(channel1, channel2, 5)   
        self.conv3 = nn.Conv2d(channel2, channel3, 5)
        self.pool2 = nn.MaxPool2d(2) #过完这一层的图像大小为9*9
        # input_dim = channel2 * size * size  
        # size = ( ((28-kernal1)+1)/2 - kernal2 + 1  )/2
        fc_dim1 = channel3 * 9 * 9
    ...
    def forward(self, x):
        ...
        temp3 = self.pool2(F.relu(self.conv3(temp2)))
        temp3 = temp3.view(temp2.shape[0], -1)
        scores = (F.relu(self.fc2(F.relu(self.fc1(temp3)))))
        return scores
